...
setwd("C:/Conexao_Mata_Atlantica/Script_R_artigo_novo")
...
# Helper tables for reporting/exports
fixed_summary <- fixed_table %>%
  select(parameter, mean, sd, q025, q50, q975)

or_table <- fixed_table %>%
  filter(parameter != "(Intercept)") %>%
  select(parameter, OR, OR_lower, OR_upper, Significant)
